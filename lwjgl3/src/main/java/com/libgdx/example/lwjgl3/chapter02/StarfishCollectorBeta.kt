...
class StarfishCollectorBeta : GameBeta() {
    ...
    override fun initialize() {
        ocean = ActorBeta()
        ocean.setTexture(Texture(Gdx.files.internal("chapter02/water.jpg")))
        mainStage.addActor(ocean)

        starfish = ActorBeta()
        starfish.setTexture(Texture(Gdx.files.internal("chapter02/starfish.png")))
        starfish.setPosition(380f, 380f)
        mainStage.addActor(starfish)

        turtle = Turtle()
        turtle.setTexture(Texture(Gdx.files.internal("chapter02/turtle-1.png")))
        turtle.setPosition(20f, 20f)
        mainStage.addActor(turtle)

        sharky = Sharky()
        sharky.setTexture(Texture(Gdx.files.internal("chapter02/sharky.png")))
        sharky.setPosition(400f, 400f)
        mainStage.addActor(sharky)

        winMessage = ActorBeta()
        winMessage.setTexture(Texture(Gdx.files.internal("chapter02/you-win.png")))
        winMessage.setPosition(180f, 180f)
        winMessage.isVisible = false
        mainStage.addActor(winMessage)

        loseMessage = ActorBeta()
        loseMessage.setTexture(Texture(Gdx.files.internal("chapter02/game-over.png")))
